...
# 30 distinct colors for subjects
color_palette = [
...
with app.app_context():
    try:
        # Create color column on Aiven if not exists
        with db.engine.connect() as conn:
            conn.execute(db.text("""
                ALTER TABLE subjects ADD COLUMN IF NOT EXISTS color VARCHAR(7) DEFAULT '#3498db'
            """))
            conn.commit()
            print('✅ Color column added to Aiven!')
